...
export default function AddProductScreen({ route, navigation }) {
  ...
  async function save() {
    ...
    if (!storeName.trim())    { Alert.alert("Erreur", "Le nom du magasin est obligatoire."); return; }
    if (!storeAddress.trim()) { Alert.alert("Erreur", "L'adresse est obligatoire."); return; }

    setLoading(true);
    try {
      await axios.post(URL + "/products", { name: name.trim(), image: image.trim(), storeName: storeName.trim(), storeAddress: storeAddress.trim(), info: info.trim(), latitude, longitude });
      Alert.alert("Succes", "Produit ajoute !", [{ text: "OK", onPress: () => navigation.goBack() }]);
    } catch {
  ...
  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      ...
      <TextInput style={styles.input} placeholder="https://exemple.com/image.png" value={image} onChangeText={setImage} autoCapitalize="none" />
      {image.trim() ? (
      ...
      {latitude && longitude ? (
        <View style={styles.locationBox}>
          ...
          <TouchableOpacity style={styles.btnChange} onPress={() => navigation.navigate("PickLocation", { ecranRetour: "AddProduct", latitude, longitude })}>
            <Text style={styles.btnChangeText}>Changer la position</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <TouchableOpacity style={styles.btnMap} onPress={() => navigation.navigate("PickLocation", { ecranRetour: "AddProduct", latitude: 30.4727, longitude: -8.8746 })}>
      ...
      <TouchableOpacity style={styles.btn} onPress={save} disabled={loading}>
        <Text style={styles.btnText}>{loading ? "Enregistrement..." : "Ajouter le produit"}</Text>
      </TouchableOpacity>
